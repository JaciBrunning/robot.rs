...
use gripper::Gripper;
use log::info;
use robot_rs::{start::RobotResult, robot_main, ds::{RobotControlState, ControlMode}};
  ...
  let main_loop = async {loop {
    ...
    match (state.enabled, last_enabled, state.mode) {
      (true, false, ControlMode::Autonomous) => {
        println!("Start");
        // Spawn the task
        tokio::task::spawn(two_piece_auto(elevator.clone(), drivetrain.clone(), gripper.clone()));
      },
      _ => ()
    }
