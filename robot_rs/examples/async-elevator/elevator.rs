...
use robot_rs::{actuators::motors::MotorController, control::{control_lock::{ControlLockResource, ControlLock}, pid::{PID, PIDConfig}}, sensors::distance::{DistanceSource, SimDistanceSource}, types::MinMax, models::DcMotor, time::now};
use strum::Display;
  ...
  H: DistanceSource + SimDistanceSource
{
  ...
  pub async fn run_sim(&self) {
    loop {
      ...
      let f_up = self.config.motor_model.torque(self.config.motor_model.current(motor_speed, voltage)) / self.config.spool_radius;
      ...
      // Physical limits
      if new_velocity > 0.0 && current_height + new_velocity * dt > self.config.height_limit.max {
        new_velocity = 0.0;
      } else if new_velocity < 0.0 && current_height - new_velocity * dt < self.config.height_limit.min {
        new_velocity = 0.0;
      }

      *self.sim_speed.write().await = new_velocity;
      self.config.height_sensor.write().await.set_distance(current_height + new_velocity * dt);
