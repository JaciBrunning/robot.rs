...
use elevator::{AbstractElevator, Elevator, ElevatorResult};
use futures::{future::join_all, FutureExt};
use log::info;
use nt4_rs::instance::NetworkTableInstance;
use robot_rs::{
    actuators::motors::{ClampedMotor, PWMSparkMax},
    control::{edge_detect::Edge, pid::PIDConfig},
    input::xbox::Xbox,
    models::DcMotor,
    robot_init, robot_main,
    sensors::{analog::AnalogInput, distance::NaiveDistanceSource},
    start::RobotResult,
    types::MinMax,
};
...
async fn manual_controls<'a, E: AbstractElevator<'a>>(elevator: &'a E, xbox: &Xbox) {
    ...
    while elevator
        .set_voltage(height_control.get() * 12.0, &control)
        .await
        .is_ok()
    {
...
async fn buttons<'a, E: AbstractElevator<'a> + Send + Sync>(elevator: &'a E, xbox: &Xbox) {
    ...
    futs.push(
        xbox.a()
            .edge_take(Edge::Falling)
            .run(|| move_to_height(elevator, 1.0))
            .boxed(),
    );
    futs.push(
        xbox.b()
            .edge_take(Edge::Falling)
            .run(|| manual_controls(elevator, xbox))
            .boxed(),
    );
