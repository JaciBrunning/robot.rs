use std::{collections::HashSet, cell::RefCell, sync::Arc};
...
pub trait Behaviour {
  ...
  fn on_started(&mut self) -> anyhow::Result<BehaviourRequest> { Ok(BehaviourRequest::Continue) }
  fn on_update(&mut self, dt: f64, total_time: f64) -> anyhow::Result<BehaviourRequest>;
  #[allow(unused_variables)]
  fn on_stopped(&mut self, state: &BehaviourState, total_time: f64) -> anyhow::Result<()> { Ok(()) }
  fn name(&self) -> String { "<unnamed behaviour>".into() }
}
...
impl BehaviourScheduler {
  ...
  pub fn schedule(&mut self, behaviour: Arc<RefCell<dyn Behaviour>>) -> anyhow::Result<()> {
    let mut contextualised = ContextualisedBehaviour {
      ...
      props: BehaviourProps { controlled_systems: HashSet::new() },
      total_time: 0.0,
      behaviour
    };
...
mod test {
  ...
  use super::{MockBehaviour, BehaviourState, BehaviourScheduler};
  ...
  fn test_scheduler_finish() -> anyhow::Result<()> {
    ...
    for i in 0..5 {
      mock.expect_on_update()
        .times(1)
        .withf(move |&dt, &total_time| relative_eq!(dt, 0.05) && relative_eq!(total_time, i as f64 * 0.05))
        .return_once(move |_, _| Ok(super::BehaviourRequest::Continue));
    }
    mock.expect_on_update()
      .once()
      .withf(move |&dt, &total_time| relative_eq!(dt, 0.05) && relative_eq!(total_time, 5.0 * 0.05))
      .return_once(move |_, _| Ok(super::BehaviourRequest::Finished));

    mock.expect_on_stopped()
      .withf(move |state, &total_time| relative_eq!(total_time, 5.0 * 0.05) && match state { BehaviourState::Done => true, _ => false })
      .return_once(move |_, _| Ok(()));
  
    let mut scheduler = BehaviourScheduler::new();
    for _ in 0..5 { scheduler.update(0.05)?; }    // Should not have any calls
    scheduler.schedule(Arc::new(RefCell::new(mock)))?;
    for _ in 0..10 { scheduler.update(0.05)?; }    // Should call 5 times
  ...
  fn test_scheduler_interrupted() -> anyhow::Result<()> {
    ...
    mock.expect_on_stopped()
      .withf(move |state, &total_time| relative_eq!(total_time, 3.0 * 0.05) && match state { BehaviourState::Interrupted => true, _ => false })
      .return_once(move |_, _| Ok(()));
  
    let mut scheduler = BehaviourScheduler::new();
    for _ in 0..5 { scheduler.update(0.05)?; }    // Should not have any calls
    scheduler.schedule(Arc::new(RefCell::new(mock)))?;
    for _ in 0..3 { scheduler.update(0.05)?; }    // Should call 3 times
    scheduler.interrupt_all();
    for _ in 0..3 { scheduler.update(0.05)?; }    // Shouldn't call at all
  ...
  fn test_scheduler_errored() -> anyhow::Result<()> {
    ...
    mock.expect_on_stopped()
      .withf(move |state, &total_time| relative_eq!(total_time, 5.0 * 0.05) && match state { BehaviourState::Errored(_) => true, _ => false })
      .return_once(move |_, _| Ok(()));
