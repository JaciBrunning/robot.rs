...
pub fn new(config: PIDConfig, setpoint: f64, history: f64) -> Self {
  ...
  pub fn calculate(&mut self, pv: f64, time: f64) -> &PIDMeasurement {
    ...
    let measurement: PIDMeasurement = match last {
      Some(last) => {
        ...
        let parts = [ self.config.kp * error, self.config.ki * integral_sum, self.config.kd * derivative ];
    ...
    // Evict old history
    while self.history.front().map(|x| (time - x.time) >= self.history_time ).unwrap_or(false) {
  ...
  pub fn nt_update(&mut self, path: &str) {
    ...
    nt!(&format!("{}/{}", basepath, "process_variable"), last.process_variable).unwrap();
    nt!(&format!("{}/{}", basepath, "error"), last.error).unwrap();
    nt!(&format!("{}/{}", basepath, "derivative"), last.derivative).unwrap();
    nt!(&format!("{}/{}", basepath, "integral_sum"), last.integral_sum).unwrap();
    nt!(&format!("{}/{}", basepath, "output"), last.output).unwrap();
    nt!(&format!("{}/{}", basepath, "output_parts"), last.output_parts.to_vec()).unwrap();
  }
}
