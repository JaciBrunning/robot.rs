...
use crate::nt_internal::{NT_Entry, NT_GetEntry, NT_GetEntryType, NT_GetEntryName, NT_Value, NT_GetEntryValue, NT_SetEntryValue};
...
impl Entry {
  pub fn new(inst: &NetworkTableInstance, name: &str) -> Self {
    ...
    Self {
      handle: unsafe {
        NT_GetEntry(inst.handle, cstr.as_ptr(), cstr.as_bytes().len())
      }
    }
  }
