...
impl DeployCommand {
  pub fn invoke(args: DeployArgs, dry_run: bool) -> CargoRobotResult<()> {
    ...
    if !dry_run {
      ...
      Err(
        Error::new("Could not connect to target.")
  ...
  fn try_deploy(
    ...
    // Deploy our program
    session
      .run(". /etc/profile.d/natinst-path.sh; /usr/local/frc/bin/frcKillRobot.sh -t 2> /dev/null")
      .ok();
  ...
  fn get_robot_metadata_entry(package: &Package, key: &str) -> CargoRobotResult<serde_json::Value> {
    match package.metadata.get("robot") {
      ...
      None => Err(
        Error::new("Missing [metadata.robot]")
    ...
    for package in &meta.packages {
      if let Some(_meta) = package.metadata.as_object() {
        let (_header_dirs, _link_dirs, runtime_dirs) =
          maybe_download_libs(package, "arm-unknown-linux-gnueabi", profile).map_err(|e| {
            Error::new("Failed to Download and Extract Dependency Libs")
              .with_remediation("Try running `cargo robot prepare` to download files ahead of time")
